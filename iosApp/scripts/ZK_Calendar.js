...
var ZK_Calendar = {
	...
	template:[
		"<div class='cal_container'>",
				"<div class='cal_top'><a class='l-arrow' ></a><span></span><a class='r-arrow'></a></div>",
				"<div class='cal_bottom'><div class='cal_menu mui-clearfix'><ul>" ,
				"<li>日</li>",
				"<li>一</li>",
				"<li>二</li>",
				"<li>三</li>",
				...
				"<li>六</li>",
				"</ul></div>",
				"<div class='cal_main mui-clearfix'></div></div>",
		"</div>"
	].join(""),
	/*日历容器*/
	container:null,
	...
	init:function(id,config){
		...
		if(config){
			...
			/*绑定当前选项鼠标经过该事件*/
			this.onMouseOver = config.onMouseOver || null;
	...
	updataCalendar:function(){
		var dt = this.container.getElementsByClassName("cal_top")[0].getElementsByTagName("span")[0];
		var dd = this.container.getElementsByClassName("cal_main")[0];
		...
		for(i=0;i<len;i++){
			...
			if(!value){
				...
				_Ctrl.addEvent(a,"mouseover",function(e){
					...
					that.target(e).className = "mouseover " + cstyle;
				})
				...
				/*绑定当前选项鼠标经过事件*/
				if(that.onMouseOver != null){
					_Ctrl.addEvent(a,"mouseover",function(e){
						var ele = that.target(e);
						that.onMouseOver(ele);
					})
				}
			}
	...
	getMonthtoDays:function(y,m){
		...
		var len = Math.ceil((days+firstDay)/7) * 7;
		/*if(len != 35){
			len = 35;
		}*/
	...
	initMouseEvent:function(){
		var that = this,
			aLeft = that.container.getElementsByClassName("cal_top")[0].getElementsByTagName("a")[0],
			aRigth = that.container.getElementsByClassName("cal_top")[0].getElementsByTagName("a")[1];
